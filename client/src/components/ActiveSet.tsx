...
export function ActiveSet({ 
  ...
  return (
    <div className="flex flex-col items-center gap-6">
      <div className="text-muted-foreground uppercase text-[10px] font-black tracking-[0.2em] opacity-40">Active Set</div>
      {activeSet ? (
        <div className="flex gap-3 p-6 bg-card border rounded-3xl shadow-xl">
          {activeSet.cards.map((card, idx) => (
            <div key={idx} className="relative group">
              ...
              {isMyTurn && activeSet?.ownerId !== socketId && !scoutingCard && (idx === 0 || idx === activeSet.cards.length - 1) && (
                <Button 
      ...
      ) : <div className="text-muted-foreground italic text-sm opacity-20 py-8">Empty Table</div>}
    </div>
  );
}
