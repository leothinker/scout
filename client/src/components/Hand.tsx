        ...
        {me.hand.map((card, idx) => (
          <React.Fragment key={idx}>
            ...
            >
              ...
              <div className="text-[10px] text-muted-foreground self-end rotate-180 font-black opacity-30">{card.bottom}</div>
            </div>
      ...
      {scoutingCard && (
        <div className="mt-8 flex flex-col items-center gap-3 pointer-events-auto bg-card px-6 py-3 rounded-full border shadow-xl">
          <div className="text-primary font-black text-xs uppercase tracking-widest">
            INSERTING: <span className="text-lg bg-primary text-primary-foreground px-2 py-1 rounded ml-2">{scoutingCard.card.top}</span> (Bottom: {scoutingCard.card.bottom})
          </div>
          <button className="text-[10px] uppercase font-black tracking-widest text-muted-foreground hover:text-primary transition-colors" onClick={onCancelScout}>Cancel Scout</button>
        </div>
      )}
    </div>
  );
